...
public:
    ...
    bool plan(std::vector<double> start_, std::vector<double> goal_, int n){
        ...
        if(sampler == 2){
            ss_->getSpaceInformation()->setValidStateSamplerAllocator(allocGaussianValidStateSampler);
        }
        if(sampler == 3){
            ss_->getSpaceInformation()->setValidStateSamplerAllocator(allocMaximizeClearanceValidStateSampler);
        }
        if(sampler == 4){
...
int main(int argc, char ** argv){
    ...
    for(int i = 0; i < N; i++){
        cout << i <<  endl;
        PRMtester tester(b1,b2,arena_parts, robot_parts, obstacles, planner_used,sampler_used);
