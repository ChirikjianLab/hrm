...
def load_results(dim, result_path, config_path):
    try:
        ...
        x_mink = pd.read_csv(result_path + "mink_bound_" + dim + ".csv",
                             delimiter='\s+', header=None, dtype=np.float).values
        cf_seg = pd.read_csv(result_path + "segment_" + dim + ".csv",
                             delimiter='\s+', header=None, dtype=np.float).values
    except:
        x_origin = None
        x_mink = None
    ...
    return x_origin, x_mink, cf_seg, vtx, edge, path, robot_config, end_pts
