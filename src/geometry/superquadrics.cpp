...
MatrixXd SuperQuadrics::originShape() {
  ...
  Vector3d C;
  C << Shape.pos[0], Shape.pos[1], Shape.pos[2];

  // Parameterized surface
  x = Shape.a[0] * expFun_mat(eta, Shape.eps[0], 0)
                       .cwiseProduct(expFun_mat(omega, Shape.eps[1], 0));
  x.resize(1, num);
  y = Shape.a[1] * expFun_mat(eta, Shape.eps[0], 0)
                       .cwiseProduct(expFun_mat(omega, Shape.eps[1], 1));
  y.resize(1, num);
  z = Shape.a[2] * expFun_mat(eta, Shape.eps[0], 1)
                       .cwiseProduct(MatrixXd::Constant(n, n, 1));
  z.resize(1, num);
  X.row(0) = x;
  X.row(1) = y;
  X.row(2) = z;
...
MatrixXd SuperQuadrics::minkSum3D(shape shp_b, int K) {
  if ((shp_b.eps[0] != 1.0) || (shp_b.eps[1] != 1.0)) {
    cerr << "Second object is not an ellipsoid" << endl;
  }
  ...
  double a1 = Shape.a[0], b1 = Shape.a[1], c1 = Shape.a[2], eps1 = Shape.eps[0],
         eps2 = Shape.eps[1], a2 = shp_b.a[0], b2 = shp_b.a[1], c2 = shp_b.a[2];
  MatrixXd R1 = Shape.q.toRotationMatrix(), R2 = shp_b.q.toRotationMatrix();

  DiagonalMatrix<double, 3> diag;
  diag.diagonal() = Array3d(a2, b2, c2);

  Matrix3d Tinv = R2 * diag * R2.transpose();

  // Gradient
  gradPhix = expFun_mat(eta, 2 - eps1, 0)
                 .cwiseProduct(expFun_mat(omega, 2 - eps2, 0)) /
             a1;
  gradPhix.resize(1, num);
  gradPhiy = expFun_mat(eta, 2 - eps1, 0)
                 .cwiseProduct(expFun_mat(omega, 2 - eps2, 1)) /
             b1;
  gradPhiy.resize(1, num);
  gradPhiz =
      expFun_mat(eta, 2 - eps1, 1).cwiseProduct(MatrixXd::Constant(n, n, 1)) /
      c1;
  gradPhiz.resize(1, num);
  ...
  return originShape() +
         (K * Tinv * Tinv * R1 * gradPhi)
             .cwiseQuotient(MatrixXd::Constant(3, 1, 1) *
                            (Tinv * R1 * gradPhi).colwise().norm());
}

// For almost uniform sampling //
// Sampe angles
vector<double> SuperQuadrics::sampleSE(double a, double b, double ep,
                                       double D) {
