...
% shortest path
plot3([start(1) vtx(path(1),1)],...
    [start(2) vtx(path(1),2)],...
    sc*[start(3) vtx(path(1),3)], 'c', 'LineWidth', 2)
plot3([goal(1) vtx(path(end),1)],...
    [goal(2) vtx(path(end),2)],...
    sc*[goal(3) vtx(path(end),3)], 'c', 'LineWidth', 2)

for i = 1:size(path,2)-1
    plot3([vtx(path(i),1) vtx(path(i+1),1)],...
        [vtx(path(i),2) vtx(path(i+1),2)],...
        sc*[vtx(path(i),3) vtx(path(i+1),3)], 'c', 'LineWidth', 2)
end
