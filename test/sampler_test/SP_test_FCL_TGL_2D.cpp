...
allocOBValidStateSampler(const ob::SpaceInformation *si) {
  return std::make_shared<ob::ObstacleBasedValidStateSampler>(si);
}

// return a GaussianValidStateSampler
ob::ValidStateSamplerPtr
allocGaussianValidStateSampler(const ob::SpaceInformation *si) {
  return std::make_shared<ob::GaussianValidStateSampler>(si);
}

// return a MaximizeClearanceValidStateSampler
ob::ValidStateSamplerPtr
allocMaximizeClearanceValidStateSampler(const ob::SpaceInformation *si) {
  return std::make_shared<ob::MaximizeClearanceValidStateSampler>(si);
}

// return a BridgeTestValidStateSampler
ob::ValidStateSamplerPtr
allocBridgeTestValidStateSampler(const ob::SpaceInformation *si) {
...
public:
  ...
  */
  ...
  double expFun(double th, double p, bool func) {
    return (func == 0) ? sgn(cos(th)) * pow(abs(cos(th)), p)
                       : sgn(sin(th)) * pow(abs(sin(th)), p);
  }

  bool readingVerticesNTriangles(std::string file_vertices,
                                 std::string file_triangles) {
   ...
   */
  bool algebraic_condition_separation(Vector2d coeff_canon_i_,
                                      Vector2d coeff_canon_j_, Vector2d r_i_,
                                      Vector2d r_j_, Matrix2d A_i_,
                                      Matrix2d A_j_) {
    // Surface i
    Matrix3d A;
    A << 1 / pow(coeff_canon_i_(0, 0), 2), 0, 0, 0,
        1 / pow(coeff_canon_i_(1, 0), 2), 0, 0, 0, -1;
    // Surface j
    Matrix3d B;
    B << 1 / pow(coeff_canon_j_(0, 0), 2), 0, 0, 0,
        1 / pow(coeff_canon_j_(1, 0), 2), 0, 0, 0, -1;

    // Rigid body transformations
    Matrix3d T_i;
    T_i << A_i_(0, 0), A_i_(0, 1), r_i_(0, 0), A_i_(1, 0), A_i_(1, 1),
        r_i_(1, 0), 0, 0, 1;

    Matrix3d T_j;
    T_j << A_j_(0, 0), A_j_(0, 1), r_j_(0, 0), A_j_(1, 0), A_j_(1, 1),
        r_j_(1, 0), 0, 0, 1;
    Matrix3d Ma = T_i;
    Matrix3d Mb = T_j;
    ...
    double T3 = (a(0, 0) * a(1, 1) * b(2, 2)) + (a(0, 0) * a(2, 2) * b(1, 1)) +
                (a(1, 1) * a(2, 2) * b(0, 0));
    double T2 = (a(0, 0) * b(1, 2) * b(2, 1)) - (a(0, 0) * b(1, 1) * b(2, 2)) -
                (a(1, 1) * b(0, 0) * b(2, 2)) + (a(1, 1) * b(0, 2) * b(2, 0)) -
                (a(2, 2) * b(0, 0) * b(1, 1)) + (a(2, 2) * b(0, 1) * b(1, 0));
    double T1 = (b(0, 0) * b(1, 1) * b(2, 2)) - (b(0, 0) * b(1, 2) * b(2, 1)) -
    ...
    // characteristic_polynomial.transpose() << endl;
  ...
  bool fcl_separation(SuperEllipse se_i_, SuperEllipse se_j_, Vector2d r_i_,
                      Vector2d r_j_, double theta_i_, double theta_j_) {
    bool res = false;

    // se_i_.printTrianglesVertices();
    // se_j_.printTrianglesVertices();

    fcl::BVHModel<fcl::OBBRSS<double>> *model_i_ =
    ...
    fcl::Matrix3<double> rotation_i_(
        fcl::AngleAxis<double>(theta_i_, fcl::Vector3<double>::UnitZ()));
    ...
    fcl::CollisionObject<double> i_superellipsoid(GeometryPtr_t(model_i_),
                                                  i_transform);

    fcl::BVHModel<fcl::OBBRSS<double>> *model_j_ =
        new fcl::BVHModel<fcl::OBBRSS<double>>();
    model_j_->beginModel();
    model_j_->addSubModel(se_j_.vertices, se_j_.triangles);
    model_j_->endModel();
    fcl::Matrix3<double> rotation_j_(
        fcl::AngleAxis<double>(theta_j_, fcl::Vector3<double>::UnitZ()));
    ...
    fcl::CollisionObject<double> j_superellipsoid(GeometryPtr_t(model_j_),
                                                  j_transform);
  ...
  void printTrianglesVertices() {
    ...
    for (int i = 0; i < vertices.size(); i++) {
      cout << vertices[i](0, 0) << " " << vertices[i](1, 0) << " "
           << vertices[i](2, 0) << endl;
    }
  }
};
...
class PRMtester {
public:
  PRMtester(double xBound, double yBound, std::vector<SuperEllipse> arena_,
            std::vector<SuperEllipse> robot_, std::vector<SuperEllipse> obs_,
            int id, int sam_) {
    arena = arena_;
    robot = robot_;
    obstacles = obs_;
    ...
    if (id_planner == 2) {
      ss_->setPlanner(
          std::make_shared<og::PRMstar>(ss_->getSpaceInformation()));
    }
    if (id_planner == 3) {
  ...
  bool compareStates(std::vector<double> goal_config,
                     std::vector<double> last_config) {
    bool res = false;
    if ((abs(last_config[0] - goal_config[0]) < 0.1) &&
        (abs(last_config[1] - goal_config[1]) < 0.1) &&
        (abs(last_config[2] - goal_config[2]) < 0.1)) {
      res = true;
    }
    return res;
  }
  bool plan(std::vector<double> start_, std::vector<double> goal_, int n) {
    ...
    if (solved) {
      ...
      outfile.open("time_fcl_sq.csv", std::ios_base::app);
      //        outfile << id_planner<<","<<sampler<<","<<flag <<","<<
      //        total_time<<"," << astar_time <<","<< construct_time<<","<<
      //        total_random<<","<<nodes_graph<<","<<edges_graph<<","<<nodes_path<<","<<valid_space<<"\n";

      outfile << id_planner << "," << sampler << "," << flag << ","
              << total_time << "," << nodes_graph << "," << edges_graph << ","
              << nodes_path << "," << valid_space << "\n";
      outfile.close();
      return true;
    }
    return false;
...
private:
  bool isStateValid(const ob::State *state) const {
    ...
    for (unsigned int j = 0; j < robot.size(); j++) {
      SuperEllipse robot_config = {
          {robot[j].a[0], robot[j].a[1], yaw, robot[j].a[3], x, y},
          robot[j].num};
      ...
      //				res = checkSeparationArena(robot_config,
      //arena[k]); 				if(res == false){ 					return res;
      //				}
      //			}
    }
    return res;
  ...
  bool checkASC(SuperEllipse robot_, SuperEllipse obs_) const {
    ...
    bool res = robot_.algebraic_condition_separation(
        coeff_canon_i_, coeff_canon_j_, r_i_, r_j_, A_i_, A_j_);
    /*if(res){
  ...
  bool checkASCArena(SuperEllipse robot_, SuperEllipse arena_) const {
    ...
    bool aux = robot_.algebraic_condition_separation(
        coeff_canon_i_, coeff_canon_j_, r_i_, r_j_, A_i_, A_j_);
    if (aux == true) {
      res = false;
    }
    ...
    /*if((coeff_canon_i_(0,0)<-coeff_canon_j_(0,0) ||
  coeff_canon_i_(0,0)>coeff_canon_j_(0,0)) &&
     (coeff_canon_i_(1,0)<-coeff_canon_j_(1,0) ||
  coeff_canon_i_(1,0)>coeff_canon_j_(1,0)) ){ res = false;
  } */
    return res;
  }
  ...
  bool checkSeparation(SuperEllipse robot_, SuperEllipse obs_) const {
    ...
    r_j_ << obs_.a[4], obs_.a[5];
    bool res =
        robot_.fcl_separation(robot_, obs_, r_i_, r_j_, robot_.a[2], obs_.a[2]);
    /*if(res){
    std::cout<<"True"<<std::endl;
  }else{
    std::cout<<"False"<<std::endl;
  ...
  bool checkSeparationArena(SuperEllipse robot_, SuperEllipse arena_) const {
    ...
    Vector2d r_j_;
    r_j_ << arena_.a[4], arena_.a[5];
    bool aux = robot_.fcl_separation(robot_, arena_, r_i_, r_j_, robot_.a[2],
                                     arena_.a[2]);
    if (aux == true) {
      res = false;
    }
...
int main(int argc, char **argv) {
  if (argc <= 7) {
    cerr << "Usage: Please add 1) configuration file 2)Agent 3)Arena "
            "4)Obstacles 5)int indicating which planner to use 6) N iteratons"
         << endl;
    return 1;
  }
  ...
  for (int j = 0; j < rob_config.size(); j++) {
    SuperEllipse robot_aux = {{rob_config[j][0], rob_config[j][1],
                               rob_config[j][2], rob_config[j][3],
                               rob_config[j][4], rob_config[j][5]},
                              b0};
  ...
  for (int j = 0; j < arena_config.size(); j++) {
    SuperEllipse arena_aux = {{arena_config[j][0], arena_config[j][1],
                               arena_config[j][2], arena_config[j][3],
                               arena_config[j][4], arena_config[j][5]},
                              b0};
  ...
  for (int j = 0; j < obs_config.size(); j++) {
    SuperEllipse obs_aux = {{obs_config[j][0], obs_config[j][1],
                             obs_config[j][2], obs_config[j][3],
                             obs_config[j][4], obs_config[j][5]},
                            b0};
  ...
  outfile.open("time_fcl_sq.csv", std::ios_base::app);
  //    outfile <<
  //    "PLANNER,SAMPLER,SUCCESS,TOTAL_TIME,ASTAR_TIME,CONST_TIME,RANDOM_CONF,GRAPH_NODES,GRAPH_EDGES,PATH_CONFIG,VALID_SPACE\n";
  outfile << "PLANNER,SAMPLER,SUCCESS,TOTAL_TIME,GRAPH_NODES,GRAPH_EDGES,PATH_"
             "CONFIG,VALID_SPACE\n";
  outfile.close();
