...
class PRMtester{
  public:
    PRMtester(double lowBound, double highBound, std::vector<SuperEllipse> arena_, std::vector<SuperEllipse> robot_, std::vector<SuperEllipse> obs_, int id){        
      arena = arena_;
      robot = robot_;
      obstacles = obs_;      
      id_planner = id;
    ...
    bool compareStates(std::vector<double> goal_config, std::vector<double> last_config){
        bool res = false;
        if((last_config[0] > goal_config[0] - 0.1 && last_config[0] < goal_config[0] + 0.1) && 
            (last_config[1] > goal_config[1] - 0.1 && last_config[1] < goal_config[1] + 0.1) &&
            (last_config[2] > goal_config[3] - 0.1 && last_config[3] < goal_config[3] + 0.1)){
            res = true;
        }
        return res;
    ...
    bool plan(std::vector<double> start_, std::vector<double> goal_, int n){
      ...
      file_state.close();
      std::vector<double> final_conf={state->as<ob::SE2StateSpace::StateType>()->getX(), 
                                        state->as<ob::SE2StateSpace::StateType>()->getY(),
                                        state->as<ob::SE2StateSpace::StateType>()->getYaw()};
      bool aux_flag = compareStates(goal_, final_conf);
      flag = int(aux_flag);
...
int main(){
    ...
    outfile.close(); 
    //Planner number: PRM:1, PRMstar:2, RRT:3, RRTconnect:4, Default(LBKPIECE):0
    int N = 1;
    for(int i = 0; i < N; i++){
        PRMtester tester(b1,b2,arena, robot, obs, 4);
