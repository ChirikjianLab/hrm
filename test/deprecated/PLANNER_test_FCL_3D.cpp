...
EMesh getMesh(SuperQuadrics sq) {
    ...
    //    for(int i=0; i<sq.num; i++)
    //    M.vertices.push_back({sq.originShape()(0,i),
    //                                                      sq.originShape()(1,i),
    //                                                      sq.originShape()(2,i)});

    //    ArrayXd q((n-1)*(n-1));
    //    for(int i=0; i<n-1; i++) q.segment(i*(n-1),(n-1)) =
    //    ArrayXd::LinSpaced(n-1,i*n,(i+1)*n-2);
    ...
    //    M.triangles.push_back({size_t(faces(i,0)),size_t(faces(i,1)),size_t(faces(i,2))});
...
class FCLtester3D {
  public:
    ...
    FCLtester3D(vector<double> lowBound, vector<double> highBound,
                ...
                vector<SuperQuadrics> obs_, vector<EMesh> obs_mesh_, int id) {
        arena = arena_;
        robot = robot_;
        obstacles = obs_;
        obs_mesh = obs_mesh_;
        ...
        if (id_planner == 4)
            ss_->setPlanner(
                std::make_shared<og::EST>(ss_->getSpaceInformation()));
        if (id_planner == 5)
            ss_->setPlanner(
    ...
    bool compareStates(std::vector<double> goal_config,
        ...
        for (size_t i = 0; i < 7; i++)
            if (abs(last_config[i] - goal_config[i]) > 0.1) res = false;
        return res;
    }
    ...
    bool isStateValid(const ob::State *state) const {
        ...
        for (unsigned int j = 0; j < robot.size(); j++) {
            ...
            for (unsigned int i = 0; i < obstacles.size(); i++) {
                bool aux = checkSeparation(rob, obj_robot[j],
    ...
    bool checkSeparation(SuperQuadrics::shape robot,
                         fcl::CollisionObject<double> obj_ellip,
                         SuperQuadrics::shape obs,
                         fcl::CollisionObject<double> obj_sq) const {
        // Ellipsoid object
        ...
        obj_sq.setTranslation(
            fcl::Vector3d(obs.pos[0], obs.pos[1], obs.pos[2]));
...
int main(int argc, char **argv) {
    ...
    outfile << "PLANNER" << ',' << "SUCCESS" << ',' << "TOTAL_TIME" << ','
            << "GRAPH_NODES" << ',' << "GRAPH_EDGES" << ',' << "PATH_CONFIG"
            << ',' << "VALID_SPACE" << endl;
    ...
    for (int j = 0; j < id_plan; j++) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            outfile << tester.id_planner << ',' << tester.flag << ','
                    << tester.total_time << ',' << tester.nodes_graph << ","
                    << tester.edges_graph << "," << tester.nodes_path << ","
                    << tester.valid_space << endl;
        }
    }
    outfile.close();
