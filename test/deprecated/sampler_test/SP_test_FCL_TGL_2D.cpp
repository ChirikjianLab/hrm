  ...
  public:
    ...
    */
    ...
    bool algebraic_condition_separation(Vector2d coeff_canon_i_,
        ...
        double T3 = (a(0, 0) * a(1, 1) * b(2, 2)) +
                    (a(0, 0) * a(2, 2) * b(1, 1)) +
                    (a(1, 1) * a(2, 2) * b(0, 0));
        double T2 =
            (a(0, 0) * b(1, 2) * b(2, 1)) - (a(0, 0) * b(1, 1) * b(2, 2)) -
            (a(1, 1) * b(0, 0) * b(2, 2)) + (a(1, 1) * b(0, 2) * b(2, 0)) -
            (a(2, 2) * b(0, 0) * b(1, 1)) + (a(2, 2) * b(0, 1) * b(1, 0));
        double T1 =
            (b(0, 0) * b(1, 1) * b(2, 2)) - (b(0, 0) * b(1, 2) * b(2, 1)) -
         ...
         * If there are more or less than 2 negative real roots, then they
         * touch.
         */
    ...
    bool fcl_separation(SuperEllipse se_i_, SuperEllipse se_j_, Vector2d r_i_,
        ...
        fcl::Transform3<double> i_transform =
        ...
        fcl::Transform3<double> j_transform =
            fcl::Transform3<double>::Identity();
...
class PRMtester {
  public:
    PRMtester(double xBound, double yBound, std::vector<SuperEllipse> arena_,
        ...
        if (id_planner == 1) {
            ss_->setPlanner(
                std::make_shared<og::PRM>(ss_->getSpaceInformation()));
        }
        if (id_planner == 2) {
            ss_->setPlanner(
        ...
        if (id_planner == 3) {
            ss_->setPlanner(
                std::make_shared<og::RRT>(ss_->getSpaceInformation()));
        }
        if (id_planner == 4) {
            ss_->setPlanner(
  ...
  private:
    ...
    bool checkSeparation(SuperEllipse robot_, SuperEllipse obs_) const {
        ...
        Vector2d r_j_;
        r_j_ << obs_.a[4], obs_.a[5];
        bool res = robot_.fcl_separation(robot_, obs_, r_i_, r_j_, robot_.a[2],
                                         obs_.a[2]);
        /*if(res){
        std::cout<<"True"<<std::endl;
      }else{
    ...
    bool checkSeparationArena(SuperEllipse robot_, SuperEllipse arena_) const {
        ...
        Vector2d r_j_;
        r_j_ << arena_.a[4], arena_.a[5];
        bool aux = robot_.fcl_separation(robot_, arena_, r_i_, r_j_,
...
int main(int argc, char **argv) {
    if (argc <= 7) {
        cerr << "Usage: Please add 1) configuration file 2)Agent 3)Arena "
                "4)Obstacles 5)int indicating which planner to use 6) N "
                "iteratons"
             << endl;
        return 1;
    }
    ...
    for (int j = 0; j < rob_config.size(); j++) {
        SuperEllipse robot_aux = {
            {rob_config[j][0], rob_config[j][1], rob_config[j][2],
    ...
    for (int j = 0; j < arena_config.size(); j++) {
        SuperEllipse arena_aux = {
            {arena_config[j][0], arena_config[j][1], arena_config[j][2],
    ...
    for (int j = 0; j < obs_config.size(); j++) {
        SuperEllipse obs_aux = {
            {obs_config[j][0], obs_config[j][1], obs_config[j][2],
    ...
    outfile
        << "PLANNER,SAMPLER,SUCCESS,TOTAL_TIME,GRAPH_NODES,GRAPH_EDGES,PATH_"
           "CONFIG,VALID_SPACE\n";
    outfile.close();
