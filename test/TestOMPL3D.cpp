...
/*
 * \brief Test script for 3D rigid-body robot planners
 *
...
int main(int argc, char** argv) {
    if (argc == 1) {
        std::cerr << "Please input in the following order: (1) time limit for "
                     ...
                     "number of SO(3) rotation samples for base (default = "
                     "60), (9) file path prefix for pre-defined rotation "
                     "samples (if random, input 0)..."
                  << std::endl;
    ...
    if (argc > 9 && strcmp(argv[9], "0") != 0) {
        quat_file = std::string(argv[9]) + '_' + std::string(argv[8]) + ".csv";
    }
