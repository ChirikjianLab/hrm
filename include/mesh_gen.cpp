...
EMesh MeshGenerator::getMesh(ParametricPoints points_){
    ...
    if ( cit == cdone ) { std::cout << "no cells" << std::endl ;}
    else {
        while(cit != cdone) {
            fcl::Triangle aux_tri0(index_of_vertex[cit->vertex(0)] ,index_of_vertex[cit->vertex(1)],index_of_vertex[cit->vertex(2)]);
            res.triangles.push_back(aux_tri0);
            fcl::Triangle aux_tri1(index_of_vertex[cit->vertex(0)] ,index_of_vertex[cit->vertex(2)],index_of_vertex[cit->vertex(3)]);
            res.triangles.push_back(aux_tri1);
            fcl::Triangle aux_tri2(index_of_vertex[cit->vertex(1)] ,index_of_vertex[cit->vertex(2)],index_of_vertex[cit->vertex(3)]);
            res.triangles.push_back(aux_tri2);
            fcl::Triangle aux_tri3(index_of_vertex[cit->vertex(0)],index_of_vertex[cit->vertex(1)],index_of_vertex[cit->vertex(3)]);
