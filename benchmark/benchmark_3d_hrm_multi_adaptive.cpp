...
int main(int argc, char ** argv){
    if (argc != 5) {
        cerr<< "Usage: Please add 1) Num of trials 2) Param for vertex 3) Num of sweep planes 4) Num of sweep lines" << endl;
        return 1;
    }
    ...
    opt.N_layers = 0; opt.N_dx = size_t(N_x); opt.N_dy = size_t(N_y);
    ...
    file_time << "SUCCESS" << ',' << "PLAN_TIME" << ',' << "N_LAYERS" << ',' <<
                 "N_X" << ',' << "N_Y" << ',' << "GRAPH_NODE" << ',' << "GRAPH_EDGE" << ',' << "PATH_NODE" << "\n";
    ...
    for(size_t i=0; i<N; i++){
        ...
        // Store results
        file_time << high3D.flag << ',' << high3D.planTime.totalTime << ',' << high3D.N_layers << ',' <<
                     N_x << ',' << N_y << ',' << high3D.vtxEdge.vertex.size() << ',' <<
