...
bool ACSmbtester::checkSeparation(SuperEllipse robot_, SuperEllipse obs_) const{      
...
//Returns true when separated and false when overlapping
bool ACSmbtester::checkSeparationArena(SuperEllipse robot_, SuperEllipse arena_) const{
    bool res = true; 
    double max = 0;
    for(int i=0;i<2;i++){
      if(robot_.a[i]>max){
